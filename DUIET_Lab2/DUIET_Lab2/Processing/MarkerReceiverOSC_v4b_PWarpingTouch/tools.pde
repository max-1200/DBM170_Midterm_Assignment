...
float mDC1 = 285*tag2screenRatio;
float mDC2 = 280*tag2screenRatio;
...
SimpleMatrix calculateHomography(PVector[] srcPoints, PVector[] dstPoints) {                  // this is the transformation to the 2D plane
...
PVector transformPoint(PVector point, SimpleMatrix homography) {                              //defines the corner points as 0,0 1,0 0,1 1,1 
  float x = point.x;
  float y = point.y;
  float z = point.z;
...
float distancePointToPlane(PVector point, PVector[] planePoints) {
  PVector normal = cross(subtract(planePoints[1], planePoints[0]), subtract(planePoints[2], planePoints[0])); // Calculate the normal vector to the plane          //caculate the distance between the object and the plane
